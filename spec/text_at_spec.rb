...
describe "#text_at" do
  ...
  it "should raise ArgumentError if :at option omitted" do
    lambda { @pdf.text_at("hai", { }) }.should.raise(ArgumentError)
  end

  it "should raise ArgumentError if :align option included" do
    lambda { @pdf.text_at("hai", :at => [0, 0], :align => :center) }.should.raise(ArgumentError)
  end

  it "should default to 12 point helvetica" do
    @pdf.text_at("Blah", :at => [100,100])
  ...
  it "should allow setting font size" do
    @pdf.text_at("Blah", :at => [100,100], :size => 16)
    text = PDF::Inspector::Text.analyze(@pdf.render)
  ...
  it "should allow overriding default font for a single instance" do
    ...
    @pdf.text_at("Blah", :size => 11, :at => [0, 0])
    @pdf.text_at("Blaz", :at => [0, 0])
    text = PDF::Inspector::Text.analyze(@pdf.render)
  ...
  it "should allow registering of built-in font_settings on the fly" do
    ...
    @pdf.text_at("Blah", :at => [100,100], :at => [0, 0])
    @pdf.font "Courier"
    @pdf.text_at("Blaz", :at => [150,150], :at => [0, 0])
    text = PDF::Inspector::Text.analyze(@pdf.render)
  ...
  if "spec".respond_to?(:encode!)
    ...
    it "should raise an exception when a utf-8 incompatible string is rendered" do
      ...
      lambda { @pdf.text_at(str, :at => [0, 0]) }.should.raise(ArgumentError)
    end
    it "should not raise an exception when a shift-jis string is rendered" do
      datafile = "#{Prawn::BASEDIR}/data/shift_jis_text.txt"
      sjis_str = File.open(datafile, "r:shift_jis") { |f| f.gets }
      @pdf.font("#{Prawn::BASEDIR}/data/fonts/gkai00mp.ttf")
      lambda { @pdf.text_at(sjis_str, :at => [0, 0]) }.should.not.raise(ArgumentError)
    end
  else
    ...
    it "should raise an exception when a shift-jis string is rendered" do
      ...
      lambda { @pdf.text_at(sjis_str, :at => [0, 0]) }.should.raise(ArgumentError)
    end
  end
end
